"""
vidalia_bringup_nodes.launch.py — Amiga robot interface nodes for ROS 2 Humble.
...
/amiga/vel must be available in ROS 2.  Options:
  A. Use ros1_bridge to bridge amiga_ros_bridge topics from ROS 1 Noetic
     into ROS 2 Humble (easiest for existing setups).
  B. Use a native ROS 2 gRPC client for the Amiga canbus service.
...
IMPORTANT: when use_ekf:=true you must ALSO disable ICP odometry's TF
publishing so the EKF can own the odom→base_link TF:
  In slam_rtabmap_lidar3d.launch.py, set publish_tf: false in icp_odometry.
  A convenience launch file (slam_with_ekf.launch.py) handles this automatically.

Usage
-----
  # Amiga nodes only (sensors + SLAM running separately):
  ros2 launch vidalia_bringup vidalia_bringup_nodes.launch.py

  # With EKF fusion:
  ros2 launch vidalia_bringup vidalia_bringup_nodes.launch.py use_ekf:=true
"""
...
def generate_launch_description():
    ...
    amiga_odom = Node(
        ...
        parameters=[{
            ...
            'publish_tf': False,   # ICP odometry owns odom→base_link by default
            'vel_topic': '/amiga/vel',
    ...
    vel_bridge = Node(
        ...
        parameters=[{
            ...
            'max_linear': 1.5,      # m/s — safe field speed
    ...
    # Fuses /odom (ICP) + /wheel_odom (wheels) → /odometry/filtered
    # When enabled, also broadcasts odom→base_link TF.
    ekf_node = Node(
