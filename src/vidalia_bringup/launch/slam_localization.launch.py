...
Then save the map with:
    bash ~/amiga_slam/scripts/save_map.sh
"""
...
def generate_launch_description():
    ...
    # Mem/IncrementalMemory=false  → do NOT extend the map
    # Mem/InitWMWithAllNodes=true  → load all nodes from the database at start
    rtabmap = Node(
        package='rtabmap_ros',
        executable='rtabmap',
        name='rtabmap',
        ...
        parameters=[{
            ...
            # Localization-mode memory settings
            'database_path': database_path,
            'Mem/IncrementalMemory': 'false',
            'Mem/InitWMWithAllNodes': 'true',
            # Keep same ICP settings as mapping so registration is consistent
            'Reg/Strategy': '1',
            'Icp/VoxelSize': '0.3',
            'Icp/MaxCorrespondenceDistance': '0.5',
