...
def generate_boustrophedon(corners, row_spacing: float, heading_deg: float,
    ...
    # Perpendicular to row travel (rows are separated along this axis)
    perp_rad = heading_rad + math.pi / 2.0

    # Rotate corners into the row-aligned frame so rows are horizontal
    ...
    # Generate Y positions for each pass (in rotated frame)
    ...
    for row_idx, ry in enumerate(row_ys):
        ...
        # Pull endpoints inward by start_margin
        rx_start = rx_min + start_margin
        rx_end = rx_max - start_margin
        if rx_start >= rx_end:
            continue  # row is too narrow after margin

        # Alternate direction (boustrophedon)
        if row_idx % 2 == 0:
            row_points_rot = [(rx_start, ry), (rx_end, ry)]
            travel_yaw = 0.0   # +X in rotated frame → heading_rad in map frame
        else:
            row_points_rot = [(rx_end, ry), (rx_start, ry)]
            travel_yaw = math.pi  # -X direction

        # Convert back to map frame and build PoseStamped
        map_travel_yaw = heading_rad + travel_yaw
        q = _quat_from_yaw(map_travel_yaw)

        for rx, ry_pt in row_points_rot:
            mx, my = _rotate(rx, ry_pt, heading_rad)  # rotate back to map frame
            ps = PoseStamped()
...
class FieldCoveragePlannerNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter(
            'field_corners',
            [0.0, 0.0,  20.0, 0.0,  20.0, 10.0,  0.0, 10.0],  # flat list x0,y0,x1,y1,...
        )
    ...
    def _start_cb(self, request, response):
        ...
        self.get_logger().info(
            ...
            f'heading={self.get_parameter("row_heading").value}°'
        )
